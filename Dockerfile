...
LABEL org.label-schema.description = "The Rxivist API web application, a Python-based RESTful interface built using the Bottle framework."
...
HEALTHCHECK --start-period=10s --interval=10s --timeout=10s CMD curl --fail http://localhost/api/v1/papers?page_size=3 || exit 1
